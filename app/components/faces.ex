defmodule Offgrid.Components.Faces do
  ...
  `Offgrid.MemberColor`, which needs the trip's member list, and only a component can hold a
  query.
  """
  ...
  defp faces(present, grants, user_id) do
    Enum.map(present, &Map.put(&1, :color, MemberColor.of(grants, user_id, &1.id)))
  end
