defmodule Offgrid.Entities.Sketch do
  ...
  `path` is an SVG path string built by `Offgrid.Utils.SVG.smooth_path/1` from `{lng, -lat}`
  points. `Offgrid.Components.Ink` uses it directly as the `d` attribute inside a `viewBox` set
  to the basemap's bounds, so the line is drawn without any projection. It is only stored and
  drawn, never queried, which is why one string is enough.
